...
export default class ActiveEndpoint {
  ...
  private static async getEndpointData(name: string): Promise<EndpointData> {
      if(!NativeChainInfos[0].endpoints.some(el => el.name === name)){
         ...
         if(endpointData === null) throw new Error(`name (${name})not found for endpoint`)
     ...
     * @param rest
     */
  public static async addCustomEndpoint(name: string,
    rpc: string,
    rest: string){
  ...
  public static async setActiveEndpointName(
    name: string
  ) {
    const store = new BrowserKVStore("");
    const data: EndpointData = {name: name, rpc: "", rest: ""}
    await store.set(ACTIVE_ENDPOINT_NAME, JSON.stringify(data));
  };
  ...
  public static async getActiveEndpoint() : Promise<EndpointData> => {
    ...
    return new Promise(resolve =>
      store.get(ACTIVE_ENDPOINT_NAME).then((result: any) => {
        if (typeof result === "undefined" || result === false) {
          resolve(ActiveEndpoint.getEndpointData(NativeChainInfos[0].defaultEndpoint));
        } else {
